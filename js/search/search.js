 ...
 * 설명: 장비 명세 가로 늘어짐 방지 및 우측 공간 실시간 레이드 진단 제어 아키텍처
 * 규칙: 코드를 쓸 때는 항상 초보자도 이해할 수 있게 상세한 주석을 달아줍니다.
 * ============================================================================
 */

// 💡 [초보자 가이드] 대시보드 전반에 사용되는 프리미엄 디자인 명도 및 그림자 스타일 정의 구조체입니다.
const SEARCH_UI = {
...
/**
 * 💡 [초보자 가이드] 레이아웃의 정합성을 무너뜨리는 구형 상단 잔여 메뉴(.sub-tab-menu)를 DOM 트리에서 영구 소거합니다.
 */
function removeLegacyTopMenu() {
    const legacyMenu = document.querySelector('.sub-tab-menu');
    if (legacyMenu) {
...
/**
 * 💡 [초보자 가이드] 통합 대시보드가 로드되기 전, 사용자가 마주할 검색 바 및 안내 인트로 스킨을 초기화합니다.
 */
        ...
        <div style="width: 100%; display: flex; flex-direction: column; gap: 20px;">
            ...
            <div style="display: flex; justify-content: space-between; align-items: center; background: #ffffff; border-radius: 16px; padding: 16px 24px; border: 1px solid rgba(15, 23, 42, 0.04);">
                <div style="display: flex; gap: 10px;">
                    <!-- 🚨 [캐시 원칙 보장]: 갱신 버튼을 눌렀을 때만 forceRefresh 요소를 true로 넘겨 API를 강제 재호출합니다. -->
                    <button id="refreshBtn" onclick="const nameEl = document.getElementById('res_profileName'); if(nameEl && typeof window.startOmniSearch === 'function') { window.startOmniSearch(nameEl.innerText.trim(), true); } else { alert('조회된 대상 캐릭터가 없습니다.'); }" 
...
/**
 * 💡 [메인 핵심 렌더러 함수] 대시보드 내부 인프라 기둥들을 정렬 규칙에 맞춰 실시간으로 조립합니다.
 */
window.renderSearchDetail = function(basic, stat, item, ability, symbol, dojang, union, ranking, link_skill, hexa_skill, skill, hexa_stat) {
    ...
    if (!basic || !basic.character_name) {
        container.innerHTML = `
            <div style="text-align: center; padding: 80px 0; color: #64748b; font-weight: 800; font-size: 14px; background: #ffffff; border-radius: 20px;">
                ⏳ 넥슨 OpenAPI로부터 보안 암호화 테이블을 수신 중입니다...
            </div>`;
        return;
    }
        ...
        <div style="width: 100%; display: flex; flex-direction: column; gap: 20px;">
            ...
            <div style="background: #ffffff; border-radius: 20px; padding: 26px; border: 1px solid rgba(15, 23, 42, 0.04); display: flex; align-items: center; gap: 24px;">
                <div style="width: 90px; height: 90px; background: #f8fafc; border-radius: 16px; overflow:hidden; flex-shrink: 0; border: 1px solid #e2e8f0; display:flex; align-items:center; justify-content:center;">
                    <img src="${basic.character_image || ''}" style="width:100%; object-fit:contain;" onerror="this.style.display='none'">
                </div>
                <div style="flex-grow: 1; display: flex; flex-direction: column; gap: 6px;">
                    <div style="display: flex; align-items: center; gap: 12px;">
                        ...
                        <span style="font-size: 10.5px; color: #7c3aed; font-weight: 800; background: #f3f0ff; padding: 2px 8px; border-radius: 6px;">
                            🍀 아획/메획 실시간 동기화 완료
                        </span>
                    </div>
                    ...
                    <div style="display: flex; gap: 16px; flex-wrap: wrap; align-items: center; border-top: 1px dashed #e2e8f0; padding-top: 10px; margin-top: 4px;">
                        ${[
                            {l:'전투력', v: typeof window.colorNum === 'function' ? window.colorNum(Number(power).toLocaleString()) : Number(power).toLocaleString(), c: '#0f172a'},
                            {l:'주스탯', v: mainStat},
            ...
            <div id="searchMainGridContent" style="display: grid; grid-template-columns: 240px 1fr 290px; gap: 20px; align-items: start; width: 100%; box-sizing: border-box;">
                ...
                <!-- 🟩 [2열 - 중앙]: 상단 정산 요약 및 하단 5:5 대칭 분할 레이아웃 -->
                <div style="display: flex; flex-direction: column; gap: 20px; min-width: 0;">
                    <!-- 상단 가로 고정 정산 요약 바 -->
                ...
                <!-- 🟨 [3열 - 우측]: 세부 스탯 창, 어빌리티, 워크스페이스 즐겨찾기 -->
                <div style="display: flex; flex-direction: column; gap: 20px; min-width: 0;">
...
/**
 * 💡 [초보자 가이드] 수신된 넥슨 스탯 테이블 배열을 가독성 높게 변환하여 사이드 컴포넌트에 바인딩합니다.
 */
window.renderSideDetailedStats = function(finalStatList) {
    ...
    focusStats.forEach(targetName => {
        ...
        if (match) {
            ...
            // 수치 정보 강조를 위해 눈이 편안한 다운톤 진적색 처리
            if (typeof window.colorNum === 'function') {
                valueStr = window.colorNum(valueStr);
            }
...
/**
 * 💡 [초보자 가이드] 실시간 전투력 기반 주요 보스전 성적표 진단 보고서를 출력합니다.
 */
window.renderDynamicBossAndSpecAdvisor = function(totalStarforce, legendCount, uniqueCount, epicCount) {
    ...
    bossList.forEach(b => {
        ...
        if (numericPower >= b.cut) {
            ...
            statusBadge = `<span style="font-size:9.5px; font-weight:900; color:#b91c1c; background:#fee2e2; padding:2px 6px; border-radius:4px; border:1px solid #fecaca;">🔴 스펙업 필요</span>`;
        }
    ...
    if (epicCount > 0) {
        adviceText = `현재 프리셋에 에픽 등급 장비가 ${window.colorNum(epicCount + "개")} 포착되었습니다. 효율적인 데미지 스케일링을 위해 유니크 등급 전환을 최우선 권장합니다.`;
    } else if (totalStarforce < 300) {
        adviceText = `잠재능력 대비 순수 스타포스 총합(${window.colorNum("★" + totalStarforce)})이 소폭 정체되어 있습니다. 17성 부위의 18성 확정 강화를 통해 기본 공/마 성능 격차 돌파를 권장합니다.`;
    } else {
        adviceText = "전반적인 부위가 상위 매체 안정권에 근접했습니다. 에디셔널 잠재능력 세부 조율 및 6차 헥사 스킬 성장에 집중해보세요.";
    }
...
/**
 * 💡 [초보자 가이드] 선택된 프리셋 번호에 해당하는 장비 카드 리스트를 한 줄식 정밀 바인딩합니다.
 */
window.switchItemPreset = function(num) {
    ...
    if (listGrid) {
        ...
        equipList.forEach(itemInfo => {
            ...
            let starHtml = (sf > 0) ? `${window.colorNum("★" + sf)}` : '';
            let potStr = "";
        ...
        summaryBox.innerHTML = `
            <div style="display:grid; grid-template-columns:1fr 1.5fr; gap:12px;">
                <div style="background:white; border:1px solid #e2e8f0; padding:8px 12px; border-radius:8px; text-align:left;">
                    ...
                    <span>${window.colorNum("★ " + totalStarforce + "개")}</span>
                </div>
...
window.switchAbilityPreset = function(num) {
    ...
    let html = `<div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px; border-bottom:1px dashed #e2e8f0; padding-bottom:6px;"><span style="color:#475569; font-size:11px; font-weight:800;">보유 명성치</span><span>${window.colorNum(fame)}</span></div>`;
    ...
    symbolData.symbol.forEach(s => {
        ...
        html += `
            <div style="background:#f8fafc; border:1px solid #e2e8f0; border-radius:6px; padding:6px 10px; display:flex; align-items:center; justify-content:space-between;">
                ...
                <span>${window.colorNum("Lv." + s.symbol_level)}</span>
            </div>
        `;
    });
...
window.renderRecentSearchesMain = function() {
    ...
    history.forEach(char => {
        ...
        `;
        wrapper.onclick = () => {
            if(typeof window.startOmniSearch === 'function') window.startOmniSearch(char.name, false);
        };
...
window.renderFavorites = function() {
    ...
    favBox.innerHTML = favorites.length === 0 
        ...
        : favorites.map(name => `
            <div onclick="if(typeof window.startOmniSearch === 'function') window.startOmniSearch('${name}', false);" style="background:#f8fafc; border:1px solid #e2e8f0; padding:10px 14px; border-radius:10px; cursor:pointer; display:flex; justify-content:space-between; align-items:center; margin-bottom:6px; transition:all 0.15s;">
...
window.executeInlineSearch = function() {
    ...
    if (charName && typeof window.startOmniSearch === 'function') {
        window.startOmniSearch(charName, false);
        inputField.value = ''; 
    }
};
