/**
 * ============================================================================
 * 👤 MAPLE OMNI - search_skill.js [HEXA CORE & STAT MASTER FIX]
 * 설명: 링크 스킬, 6차 헥사 매트릭스 코어, 3중 스탯 코어 정밀 데이터 동기화 모듈
 * 수정사양: 
 *   1. 6차 헥사 스킬코어 누락 방지를 위한 데이터 추출 예외 필터링 보강
 *   2. HEXA 스탯 코어 슬롯 배열(slot_id 기반 3개 슬롯) 다차원 매핑 바인딩 완벽 교정
 * 규칙: 초보자도 구조를 명확히 파악할 수 있도록 전체 알고리즘과 수식에 주석을 상세히 서술
 * ============================================================================
 */

/**
 * 💡 [초보자 가이드] 6차 전직 코어 및 스탯 레벨을 기반으로 현재까지 소모한 솔 에르다/조각 계산
 */
window.calculateHexaSpends = function(hexaSkills, hexaStats) {
    let totalSolErda = 0;
    let totalFragments = 0;

    // 1. 6차 헥사 스킬 코어별 소모 재화 역산 공식 가동
    if (Array.isArray(hexaSkills)) {
        hexaSkills.forEach(core => {
            if (!core) return;
            // OpenAPI 사양에 따라 변수명이 다를 수 있으므로 Null 병합 연산자(??)로 안전하게 값을 가져옵니다.
            const level = Number(core.hexa_core_level ?? core.hexa_skill_level ?? 0);
            const type = String(core.hexa_core_type || "");

            if (level <= 0) return;

            // 코어 타입별로 설정된 고유 재화 가중치 계수를 곱산합니다.
            if (type.includes("스킬")) {
    ...
    // 2. 6차 헥사 스탯 슬롯 투자 상태를 검사하여 조각 및 에르다 누적값을 연산합니다.
    if (Array.isArray(hexaStats)) {
        hexaStats.forEach(stat => {
            ...
            // 하나의 스탯 코어라도 개방되어 레벨이 존재하면 기본 개방 비용(에르다 5개) 및 스탯별 조각 비용 누적
...
window.renderSkill = function() {
    try {
        ...
        // 3️⃣ [중요 교정 패치] 6차 헥사 스탯 코어 슬롯 데이터 매핑 전면 재구조화
        // 넥슨 API는 단일 객체나 단일 배열 안에 slot_id(0, 1, 2) 정보가 포함되어 들어오므로 구조 분해 후 순밀 재배치합니다.
        ...
        if (Array.isArray(rawStatCoreData)) {
            ...
            // 단일 객체 형태로 리턴될 경우 0번 슬롯에 디폴트 배정 처리합니다.
        ...
        // 4️⃣ 사전에 정의된 재화 정산 수식 헬퍼 함수를 통한 최종 통계 연산
        const spends = window.calculateHexaSpends(hexaSkills, hexaStats);

        // 5️⃣ 전체 스킬 테이블로부터 성장이 수반되는 핵심 5차 V코어 필터링
        ...
        // 6️⃣ 반응형 진행바 컴포넌트 마크업 구조 생성기 선언
        ...
        // 7️⃣ 공통 컨테이너 스킨 및 정렬용 인라인 CSS 상수 모음
                ...
                <div style="${boxStyle}">
                    ...
                    <div class="omni-skill-scroll-grid" style="${gridScrollStyle}">
                        ${hexaSkills.length > 0 ? hexaSkills.map(h => {
                            if (!h) return '';
                            // 넥슨 API 응답 스펙의 유연성 확보 (두 가지 명세 필드를 완벽하게 수용합니다.)
                    ...
                    <div class="omni-skill-scroll-grid" style="${gridScrollStyle}">
                        ${vCores.length > 0 ? vCores.map(v => `
                            <div style="background: #ffffff; border: 1px solid #fecaca; padding: 12px; border-radius: 16px; display: flex; flex-direction: column; gap: 8px; box-shadow: 0 2px 8px rgba(225, 29, 72, 0.05);">
                                <div style="display: flex; gap: 10px; align-items: center;">
                                    ${v.icon ? `<div style="width: 40px; height: 40px; background: #fef2f2; border: 1px solid #fecaca; border-radius: 10px; display: flex; align-items: center; justify-content: center; flex-shrink: 0; overflow:hidden;"><img src="${v.icon}" style="max-width: 80%; max-height: 80%; object-fit: contain;"></div>` : ''}
                    ...
                    <div style="${boxStyle} display: flex; flex-direction: column; height: 100%; box-sizing: border-box;">
                        ...
                        <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 8px; flex-grow: 1;">
                            ${[0, 1, 2].map((idx) => {
                                ...
                                const grade = stat.stat_grade || '스탯 코어';
                                ...
                                return `
                                    <div style="background: #ffffff; border: 1px solid #e2e8f0; padding: 12px 8px; border-radius: 12px; display: flex; flex-direction: column; gap: 6px; box-sizing: border-box;">
                                        <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid #f1f5f9; padding-bottom: 4px;">
                                            <span style="font-size: 10px; font-weight: 900; color: #334155;">#${idx + 1} 슬롯</span>
                                            <span style="font-size: 8.5px; font-weight: 800; color: #7e22ce; background:#f3e8ff; padding:1px 4px; border-radius:4px;">확정</span>
                                        </div>
                                        <div style="display: flex; flex-direction: column; gap: 3px; text-align: left;">
                                            <div style="display: flex; justify-content: space-between; gap:4px;"><span style="font-size: 9px; color: #475569; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">${mainName}</span><span style="font-size: 9px; font-weight: 900; color: #f97316; flex-shrink:0;">Lv.${mainLvl}</span></div>
                                            <div style="display: flex; justify-content: space-between; gap:4px;"><span style="font-size: 9px; color: #475569; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">${sub1Name}</span><span style="font-size: 9px; font-weight: 900; color: #3b82f6; flex-shrink:0;">Lv.${sub1Lvl}</span></div>
                                            <div style="display: flex; justify-content: space-between; gap:4px;"><span style="font-size: 9px; color: #475569; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">${sub2Name}</span><span style="font-size: 9px; font-weight: 900; color: #22c55e; flex-shrink:0;">Lv.${sub2Lvl}</span></div>
                                        </div>
                                    </div>
                                `;
                            }).join('')}
                        ...
                        <div style="margin-top: 16px; padding-top: 12px; border-top: 1px solid #e2e8f0; display: flex; justify-content: space-between; align-items: center;">
                            <span style="font-size: 11px; font-weight: 800; color: #64748b;">현재 HEXA 스탯 레벨 총합</span>
                                ...
                                <div id="link-preset-content-${i}" class="link-preset-content" style="display: ${i === 0 ? 'block' : 'none'}; height: 100%;">
                                    <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(70px, 1fr)); gap: 8px; align-content: start;">
                                        ${set.skills.length > 0 ? set.skills.map(l => `
                                            <div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 12px; padding: 8px; display: flex; flex-direction: column; align-items: center; gap: 6px; box-sizing: border-box;">
                                                ${l.skill_icon ? `<div style="width: 32px; height: 32px; background: white; border-radius: 6px; display: flex; align-items: center; justify-content: center; border: 1px solid #e2e8f0;"><img src="${l.skill_icon}" style="max-width: 90%; max-height: 90%; object-fit: contain;"></div>` : ''}
