/**
 * ============================================================================
 * 🛠️ MAPLE OMNI V14 - js/builder/builder.js [INTEGRATED SIMULATOR ENGINE]
 * 설명: 보스 프리셋 조건부 자동 기계 선별, 스타포스 및 주문서 수식 시뮬레이션
 * 규칙: 상세 스텟 아래 배치 가이드 및 초보자 가독성 주석 완전 고정 적용 (image_d84fc1.png 복원 사양)
 * ============================================================================
 */

// 💡 [버그 해결 코어] scanner.js나 다른 곳에서 데이터를 넘겨주지 못할 때 builder.js가 먹통이 되는 것을 막는 전역 안전망입니다.
window.convertEquipmentToBossSet = function(equipmentArray) {
    // 장비 배열이 비어있거나 없으면 빈 배열을 그대로 반환하여 에러(TypeError)를 원천 차단합니다.
    return equipmentArray || [];
};

// 💡 [초보자 가이드] 시뮬레이터 내부에서 가상으로 변경된 수치들을 누적 저장하는 임시 메모리 변수 저장소입니다.
window.omniSimMemory = {
    selectedSlot: null,      // 사용자가 시뮬레이션을 위해 인벤토리 보드에서 클릭하여 선택한 현재 장비 슬롯 이름
    selectedItem: null,      // 선택된 슬롯 내부의 실제 아이템 원본 데이터 객체
    activePreset: 2,         // 기본 활성화되어 표기될 프리셋 번호 (스냅샷 기준 프리셋 2번 지정)
    
    // 사용자가 제어 패널을 통해 수정한 임시 스펙 수치 저장소
    customStats: {
        starforce: 17,
    ...
    // 원본 대비 변경 스펙 증가량 수치 결과 (SPEC DIFF ANALYSIS 연동)
    computedDelta: {
        totalPower: 0,
        mainStat: 0,
        attackPower: 0
...
window.findOptimalBossPreset = function(itemData) {
    ...
    for (let i = 0; i < presetSlots.length; i++) {
        ...
        // 💡 아획/메획 적용 아이템이 2개 미만인 세팅을 발견하면 "순수 보스 세팅"으로 간주하고 즉각 반환!
 ...
 *      image_d84fc1.png 와 일치하는 정밀 3열 분할 구도를 생성합니다.
 */
window.initOmniBuilderTab = function() {
    ...
    // 만약 데이터가 아직 스캔되지 않았다면 가상 테스트용 디폴트 패킷 세션 자동 강제 주입 주동
    if (!searchContext) {
    ...
    rootBox.innerHTML = `
        ...
        <div class="builder-three-column-grid">
            ...
            <div style="display: flex; flex-direction: column; gap: 20px; width: 100%;">
                ...
                <!-- Card 2: [중요 지정 가이드 규칙] 상세 스텟 스코어보드 바로 수평 아랫구역 배치용 도핑 모듈 -->
                <div class="builder-master-card">
            ...
            <div style="display: flex; flex-direction: column; gap: 20px; width: 100%;">
                <div class="builder-master-card" style="text-align: center;">
                    ...
                    <div id="builder_inventory_grid" class="builder-inventory-grid-container">
                        <!-- window.renderBuilderInventoryGrid() 함수를 통해 내부 주입 자동 렌더 -->
                    </div>

                    <!-- 프리셋 체인저 하단 스위치 캡처 스타일 완벽 형상화 -->
    ...
    // 기본 그리드 장착 보드판 드로잉 루프 가동 시동
    window.renderBuilderInventoryGrid();
};

/**
 * 💡 [초보자 가이드] image_d84fc1.png 스냅샷 지표 기준 정적 무기/방어구 스텟 로우 테이블 데이터를 가공 생성합니다.
 */
...
/**
 * 💡 [초보자 가이드] 좌측 도핑 아이콘 18종 배치 그리드용 아이콘 홀더 목록을 드로잉합니다. (체크박스 스위치 연동)
 */
...
/**
 * 💡 [초보자 가이드] image_d84fc1.png 스티치 규격 기반의 5열 6행 무결성 인벤토리 장착 보드를 동적 빌드합니다.
 */
window.renderBuilderInventoryGrid = function() {
    ...
    mapLayout.forEach(cell => {
        ...
        // 현재 프리셋 활성화 데이터 풀에서 슬롯에 매칭되는 아이템 서칭 개시
...
/**
 * 💡 [초보자 가이드] 장착 보드에서 특정 슬롯 파츠를 클릭하면 우측 제어 인터페이스에 데이터 세션을 주입 바인딩합니다.
 */
window.loadBuilderSlotControlPanel = function(slotName, itemObj) {
    ...
    // 활성화 외곽선 테두리 재정리를 위한 그리드 리드로우
    window.renderBuilderInventoryGrid();
    window.runLiveCalculationLoop();
};

/**
 * 💡 [초보자 가이드] 우측 스타포스 증감 수치 단추(`[-]`, `[+]`) 클릭 시 수치 바인딩 범위를 보정 계산합니다.
 */
...
/**
 * 💡 [초보자 가이드] 수동 조작 입력값과 스타포스 변화량을 집계하여 실시간 SPEC DIFF ANALYSIS 보드판에 리포팅하는 AI 수식 코어 루프입니다.
 */
window.runLiveCalculationLoop = function() {
    ...
    // 가상 대입 시뮬레이션 산출 공식 기조 (스타포스 1성당 올스탯 +4~15, 공/마 +3~10 구간별 가중 반영 구조 설계)
    ...
    // 전투력 상승 추정 변산식 환산 대입 (AI 최적화 가중치)
    let totalPowerDelta = (totalStatDelta * 1850) + (totalAtkDelta * 12500);

    // 하단 SPEC DIFF 전광판 스코어 라인 동적 연동 매핑
...
window.switchBuilderPreset = function(presetNumber) {
    ...
    // 인벤토리 장착 보드판 리프레시 루프 가동
    window.renderBuilderInventoryGrid();
    if(window.showOmniToast) window.showOmniToast(`프리셋 ${presetNumber}번 장비 사양 카테고리를 자동 로드 전환 완료했습니다.`, "success");
};

// 💡 [초보자 가이드] 상단 내비게이션 탭의 페이지 스위칭 상호작용 발생을 안정적 캐치 연동해주는 바인딩 리스너입니다.
